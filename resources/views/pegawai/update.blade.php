...
<div class="container p-4">
    ...
    <div class="card p-4" style="width:100%;">
        <div class="card-body">
            <form onsubmit="checkFunction()" action="{{ url('administrator/pegawai/update-data/'.$value->uuid) }}" enctype="multipart/form-data"   method="POST" >
                @csrf
                <div class="row">
                    <div class="col-lg-8">
                            ...
                            <div class="col-sm-10">
                                <select name="id_unit" class="form-control" id="" >
                                    ...
                                    @foreach($unit as $v_unit)
                                        <option {{ $value->id_unit == $v_unit->id_unit ? 'selected' : '' }} value="{{ $v_unit->id_unit }}">{{ $v_unit->nama_unit }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-10">
                                <select name="id_jabatan" class="form-control" id="" required>
                                    ...
                                    @foreach($jabatan as $v_jabatan)
                                        <option {{ $value->id_jabatan == $v_jabatan->id_jabatan ? 'selected' : '' }} value="{{ $v_jabatan->id_jabatan }}">{{ $v_jabatan->nama_jabatan }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
